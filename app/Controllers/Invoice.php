...
class Invoice extends BaseController
{
	...
	public function awb($tahun, $mingguke=1)
	{
		$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, 'F4', true, 'UTF-8', false);

		// set document information
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor('InagaTeam');
		$pdf->SetTitle(sprintf('Invoice AWB per %d minggu ke-%d', $tahun, $mingguke));
		$pdf->SetSubject('Invoice');
		$pdf->SetKeywords(sprintf('Invoice, AWB,%d', $tahun));
...
*/
		...
		$pdf->Output(sprintf('invoice_awb--%d%d.pdf', $tahun, $mingguke), 'I');
	}
